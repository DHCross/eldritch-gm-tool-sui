...
export default function MonsterGenerator() {
  ...
  const getSuggestedTropes = (nature: CreatureNature, category: CreatureCategory) => {
    return CREATURE_TROPES[nature]?.[category] || [];
  };
  ...
  const suggestedTropes = getSuggestedTropes(monsterForm.nature, monsterForm.category);
...
TY: ${monsterForm.category} | TD: ${tdString} | EA: ${eaString} | HP: ${hpString} ${hpModifiers} | DR: ${drDisplay} | ST: ${monsterForm.savingThrow} | BP: ${monsterForm.battlePhase} | Movement: ${movementString} | Notes: ${monsterForm.notes || 'None'}`;
  ...
  const saveMonster = () => {
    ...
    const combinedTags = Array.from(new Set([
      ...
      'custom',
      monsterForm.storyTrope.toLowerCase()
    ].filter(Boolean)));

    const savedMonster: SavedCharacter = {
      ...
      status: {
        ...
        status_flags: [],
        gear: [], // Placeholder for future implementation
    ...
    if (selectedParty) {
      // This part needs more work if party management is to be kept
      // For now, it just shows an alert.
      const partyName = partyFolders.find(f => f.id === selectedParty)?.name || 'trope group';
      alert(`Monster "${monsterForm.name}" saved and added to ${partyName}!`);
    } else {
      alert(`Monster "${monsterForm.name}" saved!`);
    }
